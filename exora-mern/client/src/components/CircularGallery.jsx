...
function createTextTexture(gl, text, font = 'bold 30px Figtree', color = 'white') {
  ...
  if (!context) {
    // Fallback tiny canvas to avoid texImage2D errors
    canvas.width = 2;
    canvas.height = 2;
  } else {
    context.font = font;
    ...
    canvas.width = Math.max(2, textWidth + 40);
    canvas.height = Math.max(2, textHeight + 40);
    context.font = font;
...
class Title {
  ...
  createMesh() {
    ...
    this._texAspect = width / height;
    this.updateForPlaneScale();
    // lift text slightly above card to avoid z-fighting
    this.mesh.position.z = 0.01;
    this.mesh.setParent(this.plane);
  }
  updateForPlaneScale() {
    if (!this.mesh) return;
    const textHeight = Math.max(0.01, this.plane.scale.y * 0.35);
    const textWidth = textHeight * (this._texAspect || 2);
    this.mesh.scale.set(textWidth, textHeight, 1);
    // Place text centered inside the card
    this.mesh.position.y = 0;
  }
}

class Media {
  constructor({ geometry, gl, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }) {
    this.extra = 0;
  ...
  createShader() {
    // Use a subtle glass card background; ignore external image
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 1.5 + cos(p.y * 2.0 + uTime) * 1.5) * (0.06 + uSpeed * 0.4);
      ...
      fragment: `
        ...
        void main() {
          ...
          vec3 bg = mix(vec3(0.08,0.08,0.12), vec3(0.16,0.12,0.26), vUv.y);
          gl_FragColor = vec4(bg, alpha*0.9);
        }
      `,
      uniforms: {
        uSpeed: { value: 0 },
  ...
  onResize({ screen, viewport } = {}) {
    ...
    if (this.title) this.title.updateForPlaneScale();
    this.padding = 2;
...
class App {
  ...
  createRenderer() {
    ...
    Object.assign(this.gl.canvas.style, { width: '100%', height: '100%', display: 'block' });
  }
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const defaultItems = [
      { text: 'Autonomous Decision‑Making Capabilities' },
      { text: 'Massive Cost Reduction & Efficiency Gains' },
      { text: 'Scalability Without Linear Cost Increases' },
      { text: 'Superior Customer Experience & Satisfaction' },
      { text: 'Continuous Learning & Improvement' },
      { text: 'Competitive Advantage & Innovation' }
    ];
